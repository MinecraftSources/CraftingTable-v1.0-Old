...
formApp.controller('formController', ['$scope', '$http', '$location', '$window', function($scope, $http, $location, $window) {
    $scope.edit = $location.path() == "/mn2/servertype/edit";
    $scope.serverType = {};
    ...
    $scope.cancel = function() {
        $window.location.href = '/mn2/servertype/list';
    };
    ...
    $scope.remove = function () {
        if ($scope.edit == true) {
            $http.delete('/mn2/api/servertype/delete?id=' + $scope.serverType._id).success(function (data) {
                console.log(data);
                $window.location.href = '/mn2/servertype/list';
    ...
    $scope.submit = function () {
        if ($scope.edit == true) {
            $http.put('/mn2/api/servertype/save', $scope.serverType).success(function (data) {
                $window.location.href = '/mn2/servertype/list';
            }).error(function (error) {
                $scope.error = error.error;
            });
        } else {
            $http.post('/mn2/api/servertype/add', $scope.serverType).success(function (data) {
                $window.location.href = '/mn2/servertype/list';
            }).error(function (error) {
                $scope.error = error.error;
            });
